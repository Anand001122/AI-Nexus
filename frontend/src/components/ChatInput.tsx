          ...
          <div className="flex items-center justify-between mb-1.5 px-2">
            <div className="flex items-center space-x-2">
              ...
              {currentScore < 5 && message.length > 5 && (
                <span className="text-[8px] font-black text-rose-500 uppercase tracking-tighter bg-rose-500/10 px-1.5 py-0.5 rounded-md">Improve for better results</span>
              )}
            </div>
          ...
          <div className="flex items-end space-x-4">
            <div className="flex-1 relative group">
              <textarea
                ...
                className="w-full px-6 py-4 pr-32 bg-slate-50/50 dark:bg-zinc-900/50 border border-slate-200 dark:border-white/10 rounded-3xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all duration-300 disabled:opacity-50 text-slate-900 dark:text-zinc-100 shadow-inner"
                style={{ minHeight: '60px', maxHeight: '120px' }}
                rows={1}
              />

              <div className="absolute right-4 bottom-3 flex items-center space-x-1.5">
                <button
                  type="button"
                  onClick={() => {
                    if (!user?.isPremium) {
                  ...
                  className={`p-2 rounded-xl transition-all group/expert border ${isExpertMode
                ...
                >
                  <Brain className={`h-5 w-5 transition-transform ${isExpertMode ? 'scale-110' : 'group-hover/expert:scale-110'}`} />
                </button>
                ...
                <button
                  ...
                  className={`p-2 rounded-xl transition-all group/opt border ${message.length >= 5
                ...
                >
                  {isAnalyzing ? (
                    <div className="h-5 w-5 border-2 border-blue-500/20 border-t-blue-500 rounded-full animate-spin" />
                  ) : (
                    <Sparkles className="h-5 w-5 group-hover/opt:scale-110 transition-transform" />
                  )}
                </button>
                <div className="text-[8px] font-mono text-slate-400 opacity-50 w-6 text-center tabular-nums">{message.length}</div>
              </div>
            </div>
